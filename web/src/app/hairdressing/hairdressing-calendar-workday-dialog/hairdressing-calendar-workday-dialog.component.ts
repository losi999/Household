...
export class HairdressingCalendarWorkdayDialogComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.form = new FormGroup({
      ...
      start: new FormControl(this.day.dayType === CalendarDayType.Vacation ? WORKDAY_START : this.day.start ?? WORKDAY_START, [Validators.required]),
      end: new FormControl(this.day.dayType === CalendarDayType.Vacation ? WORKDAY_END : this.day.end ?? WORKDAY_END, [Validators.required]),
    });
