    ...
    const comparer = createComparer((compare) => {
      const splitsComparers = currentDocument.splits.reduce((accumulator, currentValue, index) => {
        ...
        return {
          ...
          [`splits.[${index}].product`]: compare(getProductId(currentValue.product), reassignments.product?.from === getProductId(originalSplit.product) ? reassignments.product?.to : getProductId(originalSplit.product)),
      ...
      const deferredSplitsComparers = currentDocument.deferredSplits.reduce((accumulator, currentValue, index) => {
        ...
        return {
          ...
          [`deferredSplits.[${index}].product`]: compare(getProductId(currentValue.product), reassignments.product?.from === getProductId(originalSplit.product) ? reassignments.product?.to : getProductId(originalSplit.product)),
